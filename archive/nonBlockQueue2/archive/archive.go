...
func (queue *NBQueue) Enqueue(item *TaskRequest){			//pass in pointers
	node := NewNode(item, queue)
	// fmt.Println("current", queue.ending.next)
	queue.ending.next = node
	// fmt.Println("current", queue.ending)
	queue.ending = node
	// fmt.Println("new", queue.ending)
