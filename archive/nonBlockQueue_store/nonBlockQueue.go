...
type NBQueue struct {
	...
	tail *Node
	ListSize int
	dict map[int32]interface{} //dict taking generic pointers?
	nextID int32
...
func NewNode(item interface{}, queue *NBQueue) *Node{
	node := Node{}
...
func NewNBQueue() *NBQueue{
	...
	newList.dict = make((map[int32]interface{}))

	header := NewNode("header", &newList)		//errors?
	tail := NewNode("tail", &newList)
...
func (queue *NBQueue) Enqueue(item interface{}) bool{			//pass in pointers
...
func (queue *NBQueue) Dequeue() interface{}{
	for{
...
func (queue *NBQueue) TryDequeue() int32{
	...
	if atomic.CompareAndSwapPointer(addr, old, new){
		printStatement := strconv.Itoa(int(oldFront.id))
